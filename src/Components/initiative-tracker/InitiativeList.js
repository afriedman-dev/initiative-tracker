...
class InitiativeList extends React.Component {
  ...
  render() {
    const { characters, updateInitiativeList, removeCharacter, onCharacterUpdate } = this.props;
    ...
    return (
      <div className="initList col row align-items-center">
        <button
          ...
          type="button"
        >
          <i className="fa fa-arrow-circle-left fa-2x col btn" aria-hidden="true" />
        </button>
        <div className="col-10 row">
          {!Array.isArray(characters) || !characters.length ? (
            ...
            characters.map((char, i) => (
              <div className="flip-container char-card-container col ml-auto mb-2" key={char.id}>
                <div className="flip-card" id={'flip-card-for_' + char.id}>
        ...
        >
          <i className="fa fa-arrow-circle-right fa-2x col btn" aria-hidden="true" />
        </button>
      </div>
    );
  }
